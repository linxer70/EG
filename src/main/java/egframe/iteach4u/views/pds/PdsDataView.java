...
@Route(value = "pds/dataview/:type/:type_nm/:class/:class_nm", layout = PdsLayout.class)
@PermitAll
@Uses(Icon.class)
@PreserveOnRefresh
public class PdsDataView extends SysWindow  implements WindowImpl,BeforeEnterObserver {
	...
	@Autowired
	public PdsDataView(AuthenticatedUser securityService,Iteach4uService jdbc) {
...
//			addretrieve();
	...
	public void wInsert() {
		...
		if (this.getUI().isPresent()) {
		    ...
		    String navi = "pds/dataview/"+String.valueOf(type_id) +"/"+String.valueOf(type_nm)+"/"+
		    		String.valueOf(class_id)+"/"+String.valueOf(class_nm);
		    if(currentRoute.equals(navi)) {
			    ui.navigate("pds/dataviewinsert/"+String.valueOf(0), queryParameters);
		    }else {
	...
	public void wDelete() {
		pds_file_hd row = new pds_file_hd();
	 	row = (pds_file_hd)hd_control._grid.selecteditem; 
	 	if(row==null) {
	 		return;
	 	}	 
