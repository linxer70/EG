...
@Route(value = "exm/makeview/:type/:type_nm/:class/:class_nm", layout = ExmLayout.class)
@PermitAll
@Uses(Icon.class)
@PreserveOnRefresh
public class ExmMakeView extends SysWindow  implements WindowImpl,BeforeEnterObserver {
	...
	@Autowired
	public ExmMakeView(AuthenticatedUser securityService,Iteach4uService jdbc) {
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        var params = event.getRouteParameters();
     	if(params!=null) {
             ...
             title = "문제지관리/"+title_type_nm+"/"+title_class_nm;
             setTitle(title);
	...
	public void wInsert() {
		...
		*/
		if (this.getUI().isPresent()) {
		    ...
		    String navi = "exm/makeview/"+String.valueOf(type_id) +"/"+String.valueOf(type_nm)+"/"+
		    		String.valueOf(class_id)+"/"+String.valueOf(class_nm);
		    //if(currentRoute.equals(navi)) {
		    if(id==null) {
		    	id=Long.valueOf(0);
		    }
			    ui.navigate("exm/makeviewinsert/"+String.valueOf(Long.valueOf(id)), queryParameters);
		    //}else {
