...
import egframe.iteach4u.gridForm.d_exm_mat_unit_view;
import egframe.iteach4u.gridchild.d_sys_tooltip;
import egframe.iteach4u.service.Repository.Iteach4uService;
import jakarta.annotation.security.PermitAll;

@Route(value = "exm/unitview/:type/:type_nm/:class/:class_nm", layout = ExmLayout.class)
@PermitAll
@Uses(Icon.class)
@PreserveOnRefresh
public class ExmUnitView extends SysWindow  implements WindowImpl,BeforeEnterObserver {
	...
	@Autowired
	public ExmUnitView(AuthenticatedUser securityService,Iteach4uService jdbc) {
    ...
    public void beforeEnter(BeforeEnterEvent event) {
       var params = event.getRouteParameters();
    	if(params!=null) {
            ...
            title = "문항관리/"+title_type_nm+"/"+title_class_nm;
            setTitle(title);
    ...
    public void _inithd() {
    	hd = new d_exm_mat_unit_view(sqlca);
	    ...
	    hdlayout.getStyle().set("border", "4px solid red");
...
//			addretrieve();
	...
	public void wInsert() {
		...
		*/
		if (this.getUI().isPresent()) {
		    ...
		    String navi = "exm/unitview/"+String.valueOf(type_id) +"/"+String.valueOf(type_nm)+"/"+
		    		String.valueOf(class_id)+"/"+String.valueOf(class_nm);
		    //if(currentRoute.equals(navi)) {
		    if(id==null) {
		    	id=Long.valueOf(0);
		    }
			    ui.navigate("exm/unitviewinsert/"+String.valueOf(Long.valueOf(id)), queryParameters);
		    //}else {
	...
	public void wDelete() {
		exm_mat_unit_view row = new exm_mat_unit_view();
	 	row = (exm_mat_unit_view)hd_control._grid.selecteditem; 
	 	if(row==null) {
	 		return;
	 	}	 
	...
	public void wHdDelete() {
		exm_mat_unit_view del = hd.selecteditem;
		
	 	String callFunctionQuery;
	 	Long hdid = del.getId();
	 	callFunctionQuery = "delete from exm_mat_unit_view where id = ?;";
       Jdbc.update(callFunctionQuery,hdid);
	 	callFunctionQuery = "delete from exm_mat_unit_hd where view_id = ?;";
       Jdbc.update(callFunctionQuery,hdid);
	 	callFunctionQuery = "delete from exm_mat_unit_row where view_id = ?;";
	 	Jdbc.update(callFunctionQuery,hdid);
