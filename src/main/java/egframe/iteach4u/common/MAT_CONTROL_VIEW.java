...
@Service
public class MAT_CONTROL_VIEW extends FlexLayout{
	/*
	 * 문제 콘트롤 
	 */
	...
	public List<MAT_VIEW> 			mat_unit_list = new ArrayList<>();		//문항들 목록 
	public MAT_VIEW 					mat_unit;									//문항 변수 선언 
	...
	public MAT_CONTROL_VIEW() {//빈 문제지 추가 
		_initMatControl();
	}
	public MAT_CONTROL_VIEW(AuthenticatedUser securityService,Iteach4uService _sqlca) {//빈 문제지 추가 
		sqlca = _sqlca;
		Jdbc = sqlca.getCon();
		_initMatControl();
	}
	public MAT_CONTROL_VIEW(Iteach4uService _sqlca) {
		sqlca = _sqlca;
		Jdbc = sqlca.getCon();
		_initMatControl();
	}
	public MAT_CONTROL_VIEW(Iteach4uService _sqlca,exm_mat_unit_view _view) {//Unit 에서 데이터 조회 view=1  hd 1 or 여러개  
		sqlca = _sqlca;
		Jdbc = sqlca.getCon();
		_initMatControl(_view);
	}
	public MAT_CONTROL_VIEW(Iteach4uService _sqlca,exm_mat_make_list _list) {//Make에서 데이터 조회 
		sqlca = _sqlca;
		_init();
		mcnt = 0 ;
		listitem = _list;
	...
	public void _initData(int k ) {
		...
		MAT_VIEW matunit = mat_unit_list.get(k);
		EXAM_VIEW examunit = matunit.exam_unit;
	...
	private void _getData(MAT_VIEW _matunit) {
	...
	public void _subjectChanged(Long _subject) {
		...
		if(k>0) {
			for(int i = 0 ;i < k ;i++) {
				...
				MAT_VIEW matunit = mat_unit_list.get(i);
				EXAM_VIEW examunit = matunit.exam_unit;
				unit_combo_data= sqlca.getUsyUnitCdList(subject_id);
				examunit.unit_combo_data = unit_combo_data;
				examunit.unit_combo.setItems(unit_combo_data);
...
//            ((ExmMakeInsert) parent)._addMatControl();
	...
	public boolean wUnitCreate(List<Long> unitCodes) {
		   ...
		   for (int i = 0; i < hddata.size(); i++) {
		       ...
		       MAT_VIEW mat = new MAT_VIEW(hditem);
	...
	public void wListView() {
		   ...
		   for (int i = 0; i < hddata.size(); i++) {
		       ...
		       MAT_VIEW mat = new MAT_VIEW(hditem);//여기서 인자로 문항넘버를 받아서 우츨으로 이동 
		       mat.setData(viewitem, hditem, rowlist);
		       mat._setTitile(i);
		       mat._resizeMce();
		       mat_unit_list.add(mat);
