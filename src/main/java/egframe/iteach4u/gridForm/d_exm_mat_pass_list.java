...
public class d_exm_mat_pass_list extends SysGrid<exm_mat_make_list> implements DataWindowImpl {
	public Long type_id;
	...
	public d_exm_mat_pass_list() {
    	init();
    	unvisibleColumn();
 	} 
 	
    public d_exm_mat_pass_list(@Autowired Iteach4uService dbo) {
    	sqlca = dbo;
    	init();
    	unvisibleColumn();
    }
    public void init(){
       ...
       addComponentColumn(item -> createExamIcon(item)).setHeader("문제풀기");       
    ...
    private Button createExamIcon(exm_mat_make_list _item) {
        Button exam = new Button("문제풀기");
        exam.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        exam.addClickListener(e->{
        	insertForm(_item);
        });
        return exam;
    }
    ...
    private void insertForm(exm_mat_make_list _item) {
		if (this.getUI().isPresent()) {
		    ...
		    String navi = "exm/passview/"+String.valueOf(_item.get_type_id()) +"/"+String.valueOf(_item.get_type_nm())+"/"+
		    		String.valueOf(_item.get_class_id())+"/"+String.valueOf(_item.get_class_nm());
		    if(currentRoute.equals(navi)) {
			    ui.navigate("exm/passviewinsert/"+String.valueOf(_item.getId()), queryParameters);
		    }else {
