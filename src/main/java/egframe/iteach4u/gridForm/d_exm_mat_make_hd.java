...
public class d_exm_mat_make_hd extends SysGrid<exm_mat_pass_list> implements DataWindowImpl {
	public Long type_id;
	public Long class_id;
	public Long subject_id;
	public Long unit_id;
	
	public Column<exm_mat_pass_list> update_id;
	public Column<exm_mat_pass_list> unit_cd;
	public Column<exm_mat_pass_list> tot_num;
	public Column<exm_mat_pass_list> passed_nm;
	...
	public d_exm_mat_make_hd() {
    	___init();
 	} 
 	
    public d_exm_mat_make_hd(@Autowired Iteach4uService dbo) {
    	sqlca = dbo;
    	___init();
    }
    public void ___init(){
		...
		Set<exm_mat_pass_list> selectedItems = new HashSet<>();
       ...
       addColumn(item -> item.getValue("modify")).setHeader("modify_yn").setAutoWidth(true).setSortable(true).setKey("modify").setVisible(false);
		...
		addItemDoubleClickListener(event -> {
			exm_mat_pass_list clickedItem = event.getItem(); // 더블 클릭된 아이템
    ...
    private void insertForm(exm_mat_pass_list _item) {
...
private static Component createFilterHeader(String labelText,Consumer<String> filterChangeConsumer) {
	...
	public void retrieve(String _type_cd, String _class_cd, String _subject_cd) {
		...
		dataView = null;
		selectedrow = -1;
		readBuffer = new ArrayList<exm_mat_pass_list>(getList(_type_cd, _class_cd, _subject_cd));
		originalBuffer = new ArrayList<>();
		originalBuffer = new ArrayList<>(readBuffer);//추가 
    	if(readBuffer.size()!=0) {
			dataView = setItems((List<exm_mat_pass_list>)  readBuffer);//수정
        	selectRow(0);
    	}else {
    		dataView = setItems((List<exm_mat_pass_list>)  readBuffer);//수정
        }
	...
	public List<exm_mat_pass_list> getList(String _type_cd, String _class_cd, String _subject_cd ) {
		...
		List<exm_mat_pass_list> list = new ArrayList<>();
		ResultSet rs = null;
	...
	public void retrieve(String _type_cd, String _class_cd, String _subject_cd,List<String> _unit_cd) {
		...
		dataView = null;
		selectedrow = -1;
		readBuffer = new ArrayList<exm_mat_pass_list>(getList(_type_cd, _class_cd, _subject_cd,_unit_cd));
		originalBuffer = new ArrayList<>();
		originalBuffer = new ArrayList<>(readBuffer);//추가 
    	if(readBuffer.size()!=0) {
			dataView = setItems((List<exm_mat_pass_list>)  readBuffer);//수정
	...
	public List<exm_mat_pass_list> getList(String _type_cd, String _class_cd, String _subject_cd,List<String> unitCodes) {
		String placeholders = String.join(", ", unitCodes.stream().map(s -> "?").toArray(String[]::new));		
		String sqlsyntax = "select a.no,\n"
				+ "a.no as list_no,a.passed_nm,a.exam_type,a.tot_num,a.time_limit, a.update_id,a.update_dt,ab.unit_cd,\n"
				+ "bbb.type_cd ,bbb.class_cd,bbb.subject_cd,b.name as type_nm,c.name as class_nm,d.name as subject_nm\n"
				+ "from \n"
				+ "pass_mat_make_unit as ab \n"
				+ "left outer join codess as g on (ab.unit_cd = g.no ) ,pass_mat_make_list as a "
				+ "left outer join pass_mat_make_hd as bb on (a.no = bb.list_no)\n"
				+ "left outer join exam_mat_unit_hd as bbb on (bb.hd_no = bbb.no)\n"
				+ "left outer join codeb as b on (bbb.type_cd = b.no )  \n"
				+ "left outer join codem as c on (bbb.type_cd = c.bno and bbb.class_cd = c.no)  \n"
				+ "left outer join codes as d on (bbb.class_cd = d.mno and bbb.subject_cd = d.no)  \n"
				+ "left outer join codess as f on (f.sno = d.no and bbb.unit_cd = f.no)\n"
				//+ "left outer join codess as g on (g.sno = d.no and ab.unit_cd = g.no )\n"
				+ "left outer join usy_div_cd e  on (bbb.div_cd = e.div_cd)\n"
				+ "where a.no = ab.list_no\n"
				+ "and COALESCE(bbb.type_cd,'') like ?  and COALESCE(bbb.class_cd,'') like ?  and COALESCE(bbb.subject_cd,'') like ?  \n";
		if(unitCodes.size()==0) {
			sqlsyntax = sqlsyntax + " and COALESCE(ab.unit_cd,'')  like ? "
					+ " group by  a.no ,a.passed_nm,a.exam_type,a.tot_num,a.time_limit, a.update_id,a.update_dt,\n"
					+ " bbb.type_cd ,bbb.class_cd,bbb.subject_cd,b.name ,c.name ,d.name,ab.unit_cd \n"
					+ "order by a.no desc";
		}else {
			sqlsyntax = sqlsyntax + " and COALESCE(ab.unit_cd,'')  IN (" + placeholders + ")"
					+ " group by  a.no ,a.passed_nm,a.exam_type,a.tot_num,a.time_limit, a.update_id,a.update_dt,\n"
					+ " bbb.type_cd ,bbb.class_cd,bbb.subject_cd,b.name ,c.name ,d.name,ab.unit_cd \n"
					+ "order by a.no desc";
		}
		...
		try {
			...
			preparedStatement.setString(2,_class_cd);
			preparedStatement.setString(3,_subject_cd);
			int k = unitCodes.size();
			if(k==0) {
				preparedStatement.setString(4,"%");
			}else {
		 	    for (int i = 0; i < k; i++) {
		 	    	preparedStatement.setString(i + 4, unitCodes.get(i));
	 	 	    }
			}
			rs = preparedStatement.executeQuery();
			while (rs.next()) {
	...
	public void delete() {
		exm_mat_pass_list copy = new exm_mat_pass_list(selecteditem);
		((List<exm_mat_pass_list>) deleteBuffer).add(copy);
