...
@FieldNameConstants  
public class d_exm_mat_unit_view_free extends SysFree { 
   ...
   public d_exm_mat_unit_view_free() {
	   this.addClassName("main-layout");
	   init();
   }
   public d_exm_mat_unit_view_free(Iteach4uService dbo) {
	   sqlca = dbo;
	   init();
	   addEvent();
   }
   public void init() {
      ...
      add(type_nm,class_nm,subject_combo,unit_combo,title,memo);
       ...
       binder = new BeanValidationBinder<>(pds_past_hd.class);
       binder.setBean(new pds_past_hd()); 
